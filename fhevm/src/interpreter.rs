...
use crate::parameters::{
    RLWE_COLS, LOGBASE2K, LOGN_DECOMP, LOGN_LWE, MAXMEMORYADDRESS, MAXOPSADDRESS, VMPPMAT_COLS,
    VMPPMAT_ROWS,
};
use base2k::Module;
...
impl Interpreter {
    pub fn new(module: &Module) -> Self {
        ...
        Self {
            pc: Address::new(module, LOGN_DECOMP, MAXOPSADDRESS, VMPPMAT_ROWS, VMPPMAT_COLS),
    ...
    pub fn next(&mut self, module_pbs: &Module, module_lwe: &Module, tmp_bytes: &mut [u8]) {
        ...
        // 1) Retrieve inputs
        let mut tmp_address_input: Address = Address::new(module_lwe, LOGN_DECOMP, MAXOPSADDRESS, VMPPMAT_ROWS, VMPPMAT_COLS);
        let imm_lwe: [u8; 8] = decompose(get_inputs(module_pbs, module_lwe, &self.imm, &self.register, &self.pc, &circuit_bootstrapper, &mut tmp_address_input, tmp_bytes)); 
        let rs1_lwe: [u8; 8] = decompose(get_inputs(module_pbs, module_lwe, &self.rs2, &self.register, &self.pc, &circuit_bootstrapper, &mut tmp_address_input, tmp_bytes)); 
        let rs2_lwe: [u8; 8] = decompose(get_inputs(module_pbs, module_lwe, &self.rs1, &self.register, &self.pc, &circuit_bootstrapper, &mut tmp_address_input, tmp_bytes));
...
pub fn next_tmp_bytes(module_pbs: &Module, module_lwe: &Module) -> usize{
    read_tmp_bytes(module_lwe, RLWE_COLS, VMPPMAT_ROWS, VMPPMAT_COLS) + bootstrap_address_tmp_bytes(module_pbs, module_lwe, RLWE_COLS)
}

pub fn get_inputs(module_pbs: &Module, module_lwe: &Module, location: &Memory, registers: &Memory, pc: &Address, circuit_bootstrapper: &CircuitBootstrapper, tmp_address: &mut Address, tmp_bytes: &mut [u8]) -> u32{
    let (tmp_bytes_read, tmp_bytes_bootstrap_address) = tmp_bytes.split_at_mut(read_tmp_bytes(module_lwe, RLWE_COLS, VMPPMAT_ROWS, VMPPMAT_COLS));
    let idx_lwe: u32 = location
        .read(module_lwe, pc, tmp_bytes_read);
