...
use crate::{keys::{VMKeys, VMKeysPrepared}, parameters::CryptographicParameters, ram_offset::ram_offset};
...
fn test_ram_offset<BRA: BlindRotationAlgo, BE: Backend>() where
    ...
    rs1_prep.encrypt_sk(module, rs1, &sk_glwe_prepared, &mut source_xa, &mut source_xe, scratch.borrow());
    imm_prep.encrypt_sk(module, imm, &sk_glwe_prepared, &mut source_xa, &mut source_xe, scratch.borrow());
    ...
    ram_offset(module, &mut ram, &rs1_prep, &imm_prep, &keys_prepared, scratch.borrow());

    //println!("ram: {}", ram.decrypt(module, &sk_glwe_prepared, scratch.borrow()));
    assert_eq!(rs1.wrapping_add(imm).wrapping_sub(offset), ram.decrypt(module, &sk_glwe_prepared, scratch.borrow()))
}