...
impl<BE: Backend> AddressRead<Vec<u8>, BE> {
    ...
    pub fn alloc_from_params(params: &CryptographicParameters<BE>, max_addr: u32) -> Self
    where
        Module<BE>: GGSWPreparedFactory<BE>,
    {
        Self::alloc_from_infos(params.module(), &params.ggsw_infos(), max_addr)
    }

    pub fn alloc_from_infos<M, A>(module: &M, infos: &A, max_addr: u32) -> Self
    where
        M: ModuleLogN + GGSWPreparedFactory<BE>,
        A: GGSWInfos,
    {
        let max_value_bits = (u32::BITS - max_addr.leading_zeros()).div_ceil(module.log_n() as u32);
...
impl<D: DataMut, BE: Backend> AddressRead<D, BE> {
    ...
    {
        debug_assert!(self.max_addr() > value);
